...
def contents_view(request):
    return render(request, 'ecom/contents.html')
...
def query_view(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('query_success')  # Redirect to a success page or display a success message
    else:
        form = QueryForm()

    queries = Query.objects.all()  # Fetch all queries to display in the chatbox
    return render(request, 'query_form.html', {'form': form, 'queries': queries})
